...
class SettingsDialog {
  show() {
    ...
    // Title
    const title = ModalUtils.createElement('h3', ModalStyles.darkTitle, '⚙️ Settings');
  ...
  createForm() {
    const form = ModalUtils.createElement('div', 'display: flex; flex-direction: column; gap: 16px;');
    const settings = SettingsManager.getSettings();
  ...
  createInputGroup(label, name, value, description) {
    const group = ModalUtils.createElement('div', 'display: flex; flex-direction: column; gap: 4px;');
    
    const labelEl = ModalUtils.createElement('label', 
      'font-weight: 500; color: #ffffff; font-size: 14px;', label);
    
    const input = ModalUtils.createInput(value, `
    ...
    if (description) {
      const desc = ModalUtils.createElement('small', 'color: #999999; font-size: 12px;', description);
  ...
  createCheckboxGroup(label, name, checked, description) {
    const group = ModalUtils.createElement('div', 'display: flex; align-items: flex-start; gap: 8px;');
    ...
    const labelContainer = ModalUtils.createElement('div', 'display: flex; flex-direction: column; gap: 2px;');
    const labelEl = ModalUtils.createElement('label', 
      'font-weight: 500; color: #ffffff; font-size: 14px; cursor: pointer;', label);
    
    labelEl.addEventListener('click', () => checkbox.checked = !checkbox.checked);
  ...
  createKeyboardHelp() {
    const help = ModalUtils.createElement('div', `
      background: #252526; border: 1px solid #404040; border-radius: 4px;
      padding: 16px; margin-top: 20px;
    `);
    
    const title = ModalUtils.createElement('h4', 
      'margin: 0 0 12px 0; color: #ffffff; font-size: 14px;', '⌨️ Keyboard Shortcuts');
    
    const shortcuts = ModalUtils.createElement('div', 'font-size: 12px; color: #cccccc; line-height: 1.4;');
  ...
  createButtons(form, modal) {
    const container = ModalUtils.createElement('div', 
      'display: flex; justify-content: space-between; gap: 10px; margin-top: 24px;');
    
    const leftButtons = ModalUtils.createElement('div', 'display: flex; gap: 10px;');
    const rightButtons = ModalUtils.createElement('div', 'display: flex; gap: 10px;');
    
    const resetBtn = ModalUtils.createButton('Reset to Defaults', 'secondary', true);
