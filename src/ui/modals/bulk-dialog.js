...
class BulkDialog {
  show() {
    ...
    // Title
    const title = ModalUtils.createElement('h3', ModalStyles.title, 'Bulk Alert Processing');
    
    // Instructions
    const instruction = ModalUtils.createElement('p', ModalStyles.instruction, 
      'Paste your alert IDs (space, comma, or newline separated):');
    
    // Textarea
    const textarea = ModalUtils.createTextarea('e.g., 679289778 679434984 679443707');
    
    // Buttons
    const buttonRow = ModalUtils.createElement('div', ModalStyles.buttonRow);
    const cancelBtn = ModalUtils.createButton('Cancel');
    const startBtn = ModalUtils.createButton('Start Bulk Processing', 'primary');
