...
class ImportDialog {
  ...
  show() {
    ...
    fileInput.addEventListener("change", (e) =>
      this.handleFileSelect(e, importBtn),
    );
  ...
  handleImport() {
    ...
    reader.onerror = () => {
      this.showStatus(
        "<strong>Error!</strong><br>Could not read the file. Please try again.",
        "error",
      );
    };
  ...
  showImportResult(result) {
    if (result.success) {
      this.showStatus(
        `<strong>Import Successful!</strong><br>${result.message}`,
        "success",
      );
        ...
        if (count > 0) {
          setTimeout(() => {
            UIManager.showBulkStatus(
              `Bulk mode: ${count} imported alerts loaded. Press Cmd+↓ to start`,
            );
          }, 1000);
        }
      }
      ...
      this.showStatus(
        `<strong>Import Failed!</strong><br>${result.message}`,
        "error",
      );
    }
  }
