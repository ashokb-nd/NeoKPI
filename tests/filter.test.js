...
describe('FilterManager', () => {
  ...
  describe('getFilteredNotes', () => {
    ...
    test('should filter notes using AND logic', () => {
      const mockNotes = {
        '12345': { note: 'Test note', tags: ['urgent', 'security'], alertType: 'security' },
      ...
      const result = FilterManager.getFilteredNotes(['urgent', 'security'], 'AND');
    ...
    test('should filter notes using OR logic', () => {
      ...
      const result = FilterManager.getFilteredNotes(['urgent', 'security'], 'OR');
    ...
    test('should include hashtags from note text when enabled', () => {
      const mockNotes = {
        '12345': { note: 'Test note #urgent', tags: ['security'], alertType: 'security' },
        '67890': { note: 'Another note', tags: ['urgent'], alertType: 'warning' }
      };
      ...
      expect(TagManager.extractHashtagsFromText).toHaveBeenCalledWith('Test note #urgent');
      expect(TagManager.mergeTags).toHaveBeenCalledWith(['security'], ['urgent']);
    });
  ...
  describe('getFilteredAlertIds', () => {
    ...
    test('should pass parameters correctly to getFilteredNotes', () => {
      ...
      expect(spyGetFilteredNotes).toHaveBeenCalledWith(['urgent', 'security'], 'OR', false);
    });
  });
});
