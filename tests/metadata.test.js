...
describe("MetadataManager", () => {
  ...
  test("should store metadata URL in IndexedDB", async () => {
    ...
    expect(mockIndexedDBManager.get).toHaveBeenCalledWith(CONFIG.DATABASE.STORES.METADATA_URLS, alertId);
    expect(mockIndexedDBManager.put).toHaveBeenCalledWith(CONFIG.DATABASE.STORES.METADATA_URLS, expect.objectContaining({
      alertId,
      url: metadataPath,
      downloaded: false,
  ...
  test("should store and retrieve metadata URL - integration test", async () => {
    ...
    mockIndexedDBManager.get
      ...
      .mockResolvedValueOnce({ // Second call for retrieval
        alertId,
        url: metadataPath,
        timestamp: expect.any(String),
        downloaded: false
