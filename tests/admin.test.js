...
describe("AdminTools", () => {
  beforeEach(() => {
    ...
    mockLocalStorage.hasOwnProperty = vi.fn((key) =>
      ["notes", "settings", "bulk"].includes(key),
    );
  ...
  describe("showStorageStats", () => {
    ...
    test("should show storage statistics with MetadataManager", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        "IndexedDB URLs: 100 entries (75 downloaded)",
      );
      expect(console.log).toHaveBeenCalledWith(
        "IndexedDB Metadata: 50 cached entries",
      );
      expect(console.log).toHaveBeenCalledWith(
        "IndexedDB Size: 5 MB (5120 KB)",
      );
  ...
  describe("cleanupOldData", () => {
    test("should cleanup old data successfully", async () => {
      ...
      expect(mockMetadataManager.cleanupOldEntries).toHaveBeenCalledWith(
        500,
        15,
      );
      expect(mockUIManager.showNotification).toHaveBeenCalledWith(
