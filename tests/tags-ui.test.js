...
describe("TagsUI", () => {
  ...
  describe("createCurrentAlertTagsSection", () => {
    ...
    it("should handle Enter key press with tag input", () => {
      ...
      const eventListener = mockElement.addEventListener.mock.calls.find(
        (call) => call[0] === "keydown",
      )[1];
  ...
  describe("createSearchBox", () => {
    ...
    it("should setup input and keydown event listeners", () => {
      ...
      expect(searchBox.addEventListener).toHaveBeenCalledWith(
        "input",
        expect.any(Function),
      );
      expect(searchBox.addEventListener).toHaveBeenCalledWith(
  ...
  describe("addTag", () => {
    ...
    it("should add new tag to current alert", async () => {
      ...
      expect(NotesManager.saveNote).toHaveBeenCalledWith(
        "test-alert-123",
        "note text",
  ...
  describe("updateTagsDisplay", () => {
    ...
    it("should update tags display with current tags", async () => {
      ...
      expect(NotesManager.getTags).toHaveBeenCalledWith("test-alert-123");
      expect(TagsUI.createTagChip).toHaveBeenCalledWith(
        "manual-tag",
        true,
        false,
      );
  ...
  describe("enterFilter", () => {
    ...
    it("should handle no filters selected", () => {
      ...
      expect(UIManager.showBulkStatus).toHaveBeenCalledWith(
        "No filters selected",
      );
    });
    ...
    it("should load first filtered alert", () => {
      ...
      expect(FilterManager.getFilteredAlertIds).toHaveBeenCalledWith(
        ["tag1", "tag2"],
        "AND",
        false,
      );
      expect(TagsUI.saveRecentFilterGroup).toHaveBeenCalledWith(
        ["tag1", "tag2"],
        "AND",
      );
